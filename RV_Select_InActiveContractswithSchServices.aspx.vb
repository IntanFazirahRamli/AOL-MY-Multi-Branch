...
Partial Class RV_Select_InActiveContractswithSchServices
    ...
    Protected Sub btnPrintServiceRecordList_Click(sender As Object, e As EventArgs) Handles btnPrintServiceRecordList.Click
        ...
        If GetData() = True Then
            ...
            Session.Add("ReportType", "InActiveContractswithSchServices")
            Response.Redirect("RV_InActiveContractswithSchServices.aspx")
        End If
    ...
    Private Function GetData() As Boolean
        ...
        selFormula = "{tblservicerecord1.rcno} <> 0 and {tblservicerecord1.Status} = 'O' AND {tblcontract1.Status} <> 'O' "
        Dim selection As String
        selection = ""

        Dim qry As String = ""

        qry = "SELECT tblcontract.ContractNo,recordno as ServiceRecord,tblservicerecord.SchServiceDate,tblservicerecord.TimeIn,tblservicerecord.TimeOut, tblservicerecord.LocationID, tblservicerecord.CustName, tblservicerecord.Address1"
        qry = qry + " FROM tblservicerecord inner join tblcontract on tblservicerecord.contractno=tblcontract.contractno where tblcontract.status<>'O' AND tblservicerecord.status='O'"
        ...
        qry = qry + " ORDER BY tblservicerecord.SchServiceDate"
    ...
    Public Sub WriteExcelWithNPOI(ByVal dt As DataTable, ByVal extension As String)
        ...
        Using exportData = New MemoryStream()
            ...
            If extension = "xlsx" Then
                ...
                Response.AddHeader("Content-Disposition", String.Format("attachment;filename={0}", "SchServicesForInActiveContracts.xlsx"))
                Response.BinaryWrite(exportData.ToArray())
            ElseIf extension = "xls" Then
                Response.ContentType = "application/vnd.ms-excel"
                Response.AddHeader("Content-Disposition", String.Format("attachment;filename={0}", "SchServicesForInActiveContracts.xls"))

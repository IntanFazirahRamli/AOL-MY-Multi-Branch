...
Partial Class RV_Select_ServiceRecord
    ...
    Protected Sub btnPrintServiceRecordList_Click(sender As Object, e As EventArgs) Handles btnPrintServiceRecordList.Click
        ...
        Dim qry As String = "select * from tblservicerecord,tblservicerecorddet where tblservicerecord.rcno<>0 and tblservicerecord.recordno=tblservicerecorddet.recordno"
        selection = ""
        ...
        End If
        If rdbBillStatus.SelectedValue = "Billed" Then
            selFormula = selFormula + " and {tblservicerecord1.Billyn}= 'Y'"
            If selection = "" Then
                selection = "BillStatus = Y"
            Else
                selection = selection + ", BillStatus = Y"
            End If
            qry = qry + " and tblservicerecord.billyn = 'Y'"
        ElseIf rdbBillStatus.SelectedValue = "UnBilled" Then
            selFormula = selFormula + " and {tblservicerecord1.Billyn}= 'N'"
            If selection = "" Then
                selection = "BillStatus = N"
            Else
                selection = selection + "BillStatus = N"
            End If
            qry = qry + " and tblservicerecord.billyn = 'N'"
        ElseIf rdbBillStatus.SelectedValue = "All" Then
            If selection = "" Then
                selection = "BillStatus = All"
            Else
                selection = selection + ", BillStatus = All"
            End If
        End If
        ...
        If YrStrList3.Count > 0 Then
            ...
            '  selFormula = selFormula + " and {tblservicerecorddet1.ServiceID} in [" + YrStr + "]"
            selFormula = selFormula + " and {tblcontract1.Industry} in [" + YrStr + "]"
            If selection = "" Then
                selection = "Industry : " + YrStr
            Else
                selection = selection + ", Industry : " + YrStr
            End If
            '  qry = qry + " and tblservicerecord.CompanyGroup in [" + YrStr + "]"
        End If
        ...
        If String.IsNullOrEmpty(txtBillAmtTo.Text) = False Then
            ...
            If selection = "" Then
                selection = "BillAmount <= " + txtBillAmtTo.Text
            Else
        ...
        If chkGrouping.SelectedValue = "No Grouping" Then
            Session.Add("ReportType", "ServiceRecord02")
            Response.Redirect("RV_ServiceRecord02.aspx")
        ElseIf chkGrouping.SelectedValue = "Staff" Then
            Session.Add("ReportType", "ServiceRecordStaff")

            Response.Redirect("RV_ServiceRecordStaff.aspx")
        ElseIf chkGrouping.SelectedValue = "Client" Then
            Session.Add("ReportType", "ServiceRecordClient")
    ...
    Public Sub WriteExcelWithNPOI(ByVal dt As DataTable, ByVal extension As String)
        ...
        If Convert.ToString(Session("LocationEnabled")) = "Y" Then
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                For j As Integer = 0 To dt.Columns.Count - 1
                    ...
                    ElseIf j = 10 Or j = 22 Or j = 25 Or j = 27 Then
                        If String.IsNullOrEmpty(dt.Rows(i)(j).ToString) = False Then
        ...
        Using exportData = New MemoryStream()
            ...
            If extension = "xlsx" Then
                ...
                Response.AddHeader("Content-Disposition", String.Format("attachment;filename={0}", "ServiceRecordListing.xlsx"))
                Response.BinaryWrite(exportData.ToArray())
            ElseIf extension = "xls" Then
                Response.ContentType = "application/vnd.ms-excel"
                Response.AddHeader("Content-Disposition", String.Format("attachment;filename={0}", "ServiceRecordListing.xls"))
    ...
    Public Sub InsertIntoTblWebEventLog(events As String, errorMsg As String, ID As String)
        Try
            ...
            insCmds.Parameters.AddWithValue("@LoginId", "Excel Test - " + Session("UserID"))
    ...
    Protected Sub btnExportToExcel_Click(sender As Object, e As EventArgs) Handles btnExportToExcel.Click
        ...
        If GetData() = True Then
            ...
            Dim attachment As String = "attachment; filename=ServiceRecordListing.xls"
    ...
    Private Function GetData() As Boolean
        ...
        If YrStrList3.Count > 0 Then
            ...
            Else
                selection = selection + ", Industry : " + YrStr
            End If
            qry = qry + " and tblcontract.Industry in (" + YrStr + ")"
        End If
    ...
    Public Sub DTToExcel(ByVal dt As DataTable)
        ...
        'Dim f As FileInfo = New FileInfo(Server.MapPath("Downloads") & String.Format("\{0}.xlsx", FileName))
    ...
    Protected Sub btnNewExportToExcel_Click(sender As Object, e As EventArgs) Handles btnNewExportToExcel.Click
        If GetData() = True Then
            'MessageBox.Message.Alert(Page, txtQuery.Text, "str")
            'Return

...
Partial Class RV_Select_DueForRenewalServiceContract
    ...
    Private Function GetData() As Boolean
        ...
        qry = qry + " WHERE (tblcontract1.RenewalContractNo='' or tblcontract1.RenewalContractNo is null)"

        selFormula = "{tblcontract1.rcno} <> 0 and (isnull({tblContract1.RenewalContractNo}) OR {tblContract1.RenewalContractNo}='') "
        ...
        If String.IsNullOrEmpty(txtContractDateFrom.Text) = False Then
            ...
            Else
                MessageBox.Message.Alert(Page, "Expiry Date From is invalid", "str")
                '  lblAlert.Text = "INVALID START DATE"
                Return False
            End If
            qry = qry + " and tblcontract1.EndDate >='" + Convert.ToDateTime(txtContractDateFrom.Text).ToString("yyyy-MM-dd") + "'"

            selFormula = selFormula + " and {tblcontract1.EndDate} >=" + "#" + d.ToString("MM-dd-yyyy") + "#"
            If selection = "" Then
                selection = "Expiry Date >= " + d.ToString("dd-MM-yyyy")
            Else
                selection = selection + ", Expiry Date >= " + d.ToString("dd-MM-yyyy")
            End If
        End If
        If String.IsNullOrEmpty(txtContractDateTo.Text) = False Then
            ...
            Else
                MessageBox.Message.Alert(Page, "Expiry Date To is invalid", "str")
                '  lblAlert.Text = "INVALID START DATE"
                Return False
            End If
            qry = qry + " and tblcontract1.EndDate <='" + Convert.ToDateTime(txtContractDateTo.Text).ToString("yyyy-MM-dd") + "'"
    ...
    Public Sub WriteExcelWithNPOI(ByVal dt As DataTable, ByVal extension As String)
        ...
        Using exportData = New MemoryStream()
            ...
            Dim attachment As String = "attachment; filename=DueForRenewalServiceContract"

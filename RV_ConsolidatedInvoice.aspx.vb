    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                Dim ConsolidatedInvNo As String = Convert.ToString(Session("ConsolidatedInvNo"))
                lblRecordNo.Text = ConsolidatedInvNo
                Dim Query As String = Convert.ToString(Session("Query"))
                txtQuery.Text = Query
                Dim RcNo As String = Convert.ToString(Session("RcNo"))
                txtSvcRcno.Text = RcNo
                ...
                crReportDocument.Load(Server.MapPath("~/Reports/ARReports/ConsolidatedEInvFormat.rpt"))

                crReportDocument.SetParameterValue("pLongID", Server.MapPath("~/PDFs/" + Convert.ToString(Session("ConsolidatedInvNo") + ".png")))

                Dim img As New WebClient

                If ConfigurationManager.AppSettings("DomainName").ToString() = "MALAYSIA-NEW (Beta)-V2" Then
                    img.DownloadFile("https://nossl.quickchart.io/qr?size=100&text=https://preprod.myinvois.hasil.gov.my/" & Convert.ToString(Session("UUID")) & "/share/" & Convert.ToString(Session("LongID")), Server.MapPath("~/PDFs/" + Convert.ToString(Session("ConsolidatedInvNo") + ".png")))

                Else
                    img.DownloadFile("https://nossl.quickchart.io/qr?size=100&text=https://myinvois.hasil.gov.my/" & Convert.ToString(Session("UUID")) & "/share/" & Convert.ToString(Session("LongID")), Server.MapPath("~/PDFs/" + Convert.ToString(Session("ConsolidatedInvNo") + ".png")))
                ...
                Next
                '   lblRecordNo.Text = "CAR202508-000015"
                crReportDocument.RecordSelectionFormula = "{tbleinvoiceconsolidated1.ConsolidatedInvoiceNo} = '" + lblRecordNo.Text + "'"

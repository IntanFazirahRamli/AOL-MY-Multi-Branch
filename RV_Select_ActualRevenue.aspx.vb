...
Partial Class RV_Select_ActualRevenue
    Inherits System.Web.UI.Page

    '  Dim SelFormulaText As String = ""
    ...
    Protected Sub btnPrintServiceRecordList_Click(sender As Object, e As EventArgs) Handles btnPrintServiceRecordList.Click
        ...
        Else
            selFormula = selFormula + " and {tblservicerecord1.serviceby} = '" + ddlIncharge.Text.Replace("'", "''") + "'"
            If selection = "" Then
        ...
        If chkGrouping.SelectedValue = "Date" Then
            If rbtnSelectDetSumm.SelectedValue = "1" Then
                Session.Add("rptTitle", "ACTUAL REVENUE REPORT BY DATE - DETAIL")
            ...
            ElseIf rbtnSelectDetSumm.SelectedValue = "2" Then
                Session.Add("rptTitle", "ACTUAL REVENUE REPORT BY DATE - SUMMARY")
        ...
        ElseIf chkGrouping.SelectedValue = "Client" Then
            Session.Add("selFormula1", selFormula)
            Session.Add("selection1", selection)
        ...
        ElseIf chkGrouping.SelectedValue = "PostalCode" Then
            If rbtnSelectDetSumm.SelectedValue = "1" Then
                Session.Add("rptTitle", "ACTUAL REVENUE REPORT BY POSTAL CODE - DETAIL")
                Session.Add("ReportType", "RevenueRptByPostalCode")
                Response.Redirect("RV_RevenueRptByPostalCode.aspx")
            ElseIf rbtnSelectDetSumm.SelectedValue = "2" Then
                Session.Add("rptTitle", "ACTUAL REVENUE REPORT BY POSTAL CODE - SUMMARY")
    ...
    Protected Sub btnPrintClientID_Click(sender As Object, e As EventArgs) Handles btnPrintClientID.Click
        Dim selFormula As String = Convert.ToString(Session("selFormula1"))
        Dim selection As String = Convert.ToString(Session("selection1"))
        ...
        If rbtnSelectDetSumm.SelectedValue = "1" Then
            ...
            Session.Add("rptTitle", "ACTUAL REVENUE REPORT BY CLIENT - DETAIL")

            Response.Redirect("RV_RevenueRptByClient.aspx")
        ElseIf rbtnSelectDetSumm.SelectedValue = "2" Then
            Session.Add("ReportType", "RevenueRptByClientSummary")
            Session.Add("rptTitle", "ACTUAL REVENUE REPORT BY CLIENT - Summary")
    ...
    Private Function GetData() As Boolean
        Dim selFormula As String
        selFormula = "{tblservicerecord1.rcno} <> 0 and {tblservicerecord1.Status} IN ['P'] "
        Dim selection As String
        selection = ""
        If chkDistribution.Checked Then
            ...
            Else
                cmd.CommandText = "spActualRevenueDistribution"
            ...
            cmd.Parameters.AddWithValue("pr_CreatedBy", Session("UserID"))
                 cmd.Parameters.AddWithValue("pr_ReportName", "ActualRevenue")
            ...
            If rbtnSelectDetSumm.SelectedIndex = 0 Then
                qry = " SELECT TeamID,ContractNo,RefType as RecordNo,LocationID,CustName,ServiceDate,TimeIn,TimeOut,Duration,BreakdownDuration,Address,"
                qry = qry + "AddBuilding,AddPostal,AddStreet,BillingFrequency,AgreeValue,ContractGroup,Percentage,TotalAmount as ServiceValue,BilledAmount as BreakdownServiceValue,Salesman"


            ElseIf rbtnSelectDetSumm.SelectedIndex = 1 Then
                If chkGrouping.SelectedValue = "Date" Then
                    qry = " SELECT ServiceDate,count(RefType) as NoOfServices,sum(BilledAmount) as ServiceValue"
                ElseIf chkGrouping.SelectedValue = "Client" Then
                    qry = " SELECT AccountID,count(RefType) as NoOfServices,sum(BilledAmount) as ServiceValue"
                ElseIf chkGrouping.SelectedValue = "Team" Then
                    qry = " SELECT TeamID,count(RefType) as NoOfServices,sum(BilledAmount) as ServiceValue"
                ElseIf chkGrouping.SelectedValue = "PostalCode" Then
                    qry = " SELECT AddPostal as PostalCode,count(RefType) as NoOfServices,sum(BilledAmount) as ServiceValue"
                End If
            End If
            qry = qry + ",CompanyGroup FROM tbwrevenuereport where createdby='" & Session("UserID") & "'"
            qry = qry + "And ReportName = 'ActualRevenue'"
            ...
            Else
                ...
                Else
                    selection = selection + ", ServiceBy : " + ddlIncharge.Text
                End If
                qry = qry + " and serviceby = '" + ddlIncharge.Text.Replace("'", "''") + "'"
            ...
            If String.IsNullOrEmpty(txtTeam.Text) = False Then
                ...
                Else
                    selection = selection + ", TeamID = " + txtTeam.Text
                End If
                qry = qry + " and TeamID = '" + txtTeam.Text + "'"
            ...
            If rbtnSelectDetSumm.SelectedIndex = 1 Then
                ...
                ElseIf chkGrouping.SelectedValue = "Team" Then
                    qry = qry + " group by teamid"
                ElseIf chkGrouping.SelectedValue = "PostalCode" Then
                    qry = qry + " group by addpostal"
                End If

            End If
            If chkGrouping.SelectedValue = "Date" Then
                qry = qry + " order by servicedate"
            ElseIf chkGrouping.SelectedValue = "Client" Then
                qry = qry + " order by AccountID"
            ElseIf chkGrouping.SelectedValue = "Team" Then
                qry = qry + " order by teamid"
            ElseIf chkGrouping.SelectedValue = "PostalCode" Then
                qry = qry + " order by addpostal"
            End If
        ...
        Else
            ...
            Dim qry As String = ""
            If rbtnSelectDetSumm.SelectedIndex = 0 Then
                qry = " SELECT distinct tblservicerecord.TeamID,tblcontract.ContractNo,tblservicerecord.RecordNo,tblservicerecord.LocationID,replace(replace(tblservicerecord.CustName, char(10), ' '), char(13), ' ') as CustName,tblservicerecord.ServiceDate,tblservicerecord.TimeIn,tblservicerecord.TimeOut,tblservicerecord.Duration,replace(replace(replace(tblservicerecord.Address1, char(10), ' '), char(13), ' '),'\t',' ') as Address, replace(replace(replace(tblservicerecord.AddBuilding, char(10), ' '), char(13), ' '),'\t',' ') as AddBuilding,replace(replace(replace(tblservicerecord.AddPostal, char(10), ' '), char(13), ' '),'\t','') as AddPostal, replace(replace(replace(tblservicerecord.AddStreet, char(10), ' '), char(13), ' '),'\t',' ') as AddStreet, tblcontract.BillingFrequency,tblcontract.AgreeValue,tblservicerecord.BillAmount as ServiceValue,tblcontract.Salesman"
            ...
            ElseIf rbtnSelectDetSumm.SelectedIndex = 1 Then
                ...
                End If
            End If
            qry = qry + " From tblservicerecord,tblcontract where tblservicerecord.ContractNo=tblcontract.ContractNo and tblservicerecord.status in ('P')"
            ...
            Else
                ...
                Else
                    selection = selection + ", ServiceBy : " + ddlIncharge.Text
                End If
                qry = qry + " and tblservicerecord.serviceby = '" + ddlIncharge.Text.Replace("'", "''") + "'"
    ...
    Protected Sub btnExportToExcel_Click(sender As Object, e As EventArgs) Handles btnExportToExcel.Click
        If GetData() = True Then
            ...
            Dim attachment As String = "attachment; filename=ActualRevenue.xls"
    ...
    Public Sub WriteExcelWithNPOI(ByVal dt As DataTable, ByVal extension As String)
            ...
            If chkDistribution.Checked = True Then
                ...
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    For j As Integer = 0 To dt.Columns.Count - 1
                        ...
                        If j = 15 Or j = 17 Or j = 18 Or j = 19 Or j = 8 Or j = 9 Then
        ...
        Using exportData = New MemoryStream()
            ...
            Dim criteria As String = "_" + Convert.ToDateTime(txtSvcDateFrom.Text).ToString("yyyyMMdd") + "_" + Convert.ToDateTime(txtSvcDateTo.Text).ToString("yyyyMMdd")
            ...
            Dim attachment As String = "attachment; filename=ActualRevenue"

            If extension = "xlsx" Then
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                Response.AddHeader("Content-Disposition", String.Format(attachment + criteria + ".xlsx"))

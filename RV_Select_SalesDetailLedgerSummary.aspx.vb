...
Partial Class RV_Select_SalesDetailLedgerSummary
    ...
    Protected Sub btnPrintServiceRecordList_Click(sender As Object, e As EventArgs) Handles btnPrintServiceRecordList.Click
        If GetData() = True Then
            ...
            If chkDistribution.Checked Then
                Session.Add("Distribution", "YES")
                Session.Add("InvoiceDateTo", txtInvDateTo.Text)
            End If
            ...
            ElseIf rbtnSelectDetSumm.SelectedValue = "2" Then
                Response.Redirect("RV_SalesDetailLedgerSummary.aspx")
            End If
    ...
    Public Sub InsertIntoTblWebEventLog(events As String, errorMsg As String, ID As String)
        Try
            ...
            insCmds.Parameters.AddWithValue("@LoginId", "Sales Ledger Report - " + Session("UserID"))
    ...
    Private Function GetData() As Boolean
        ...
        If chkDistribution.Checked = True Then
            ...
            'Else
            '          cmd.CommandText = "spSalesListingByLedgerDistribution"
            ...
            If rbtnSelectDetSumm.SelectedValue = "2" Then
                cmd.Parameters.AddWithValue("pr_ReportName", "SalesLedgerSummary")
            ElseIf rbtnSelectDetSumm.SelectedValue = "1" Then
                cmd.Parameters.AddWithValue("pr_ReportName", "SalesLedgerDetail")
            End If
            ...
            If rbtnSelectDetSumm.SelectedValue = "1" Then
                ...
                End If
                qry = qry + "LedgerCode,SubLedgerCode,COACode,ContactType,CompanyGroup,"
                qry = qry + "ContractGroup,Percentage,InvoiceNumber,ValueBase,GstBase"
                qry = qry + " FROM tbwsaleslistingbyledger where createdby='" & Session("UserID") & "'"
                qry = qry + "And ReportName = 'SalesLedgerDetail'"
            ...
            ElseIf rbtnSelectDetSumm.SelectedValue = "2" Then
                ...
                End If
                qry = qry + "LedgerCode,SubLedgerCode,COACode,ContactType,CompanyGroup,"
                qry = qry + "ContractGroup,sum(ValueBase) as ValueBase,sum(GstBase) as GstBase"
                qry = qry + " FROM tbwsaleslistingbyledger where createdby='" & Session("UserID") & "'"
                qry = qry + "And ReportName = 'SalesLedgerSummary'"
            ...
            If String.IsNullOrEmpty(txtInvDateTo.Text) = False Then
                ...
                'If chkDistribution.Checked Then
                '    qry = qry + " and if(isnull(tblsalesdetail.costcode)=1 or tblsalesdetail.costcode='',1=1,tblcontractvaluedistribution.distributiondate"
                '    qry = qry + " = (select max(distributiondate) from tblcontractvaluedistribution where tblcontractvaluedistribution.contractno =tblsalesdetail.costcode "
                '    qry = qry + " and tblcontractvaluedistribution.distributiondate<= '" + Convert.ToDateTime(txtInvDateTo.Text).ToString("yyyy-MM-dd") + "'))"
                '    'qry = qry + " and tblcontractvaluedistribution.distributiondate in (select max(distributiondate) from tblcontractvaluedistribution"
                '    'qry = qry + " where tblcontractvaluedistribution.contractno =tblsalesdetail.costcode and tblcontractvaluedistribution.distributiondate<= '" + Convert.ToDateTime(txtInvDateTo.Text).ToString("yyyy-MM-dd") + "')"
                '    '' selFormula = selFormula + " and {Command.DistributionDate} <=" + "#" + d.ToString("MM-dd-yyyy") + "#"
                ...
                'Else
                '    If chkDistribution.Checked Then
                '        qry = qry + " and if(isnull(tblsalesdetail.costcode)=1 or tblsalesdetail.costcode='',1=1,tblcontractvaluedistribution.distributiondate in (select max(distributiondate) from tblcontractvaluedistribution"
                '        qry = qry + " where tblcontractvaluedistribution.contractno =tblsalesdetail.costcode))"
                '    End If
            ...
            If ddlAccountType.Text = "-1" Then
            Else
                qry = qry + " and ClientType = '" + ddlAccountType.Text + "'"
                selFormula = selFormula + " and {tbwsaleslistingbyledger1.ClientType} = '" + ddlAccountType.Text + "'"
                If selection = "" Then
            ...
            If rbtnSelectDetSumm.SelectedValue = "2" Then 'summary
                qry = qry + " GROUP BY LedgerCode, SubLedgerCode, ContractGroup, ContractGroup, CompanyGroup,ContactType"
                ' qry = qry + " GROUP BY tblsalesdetail.LedgerCode"
            End If
        ...
        Else
            If rbtnSelectDetSumm.SelectedValue = "1" Then
                ...
                End If
                qry = qry + "tblsalesdetail.LedgerCode, tblsalesdetail.SubLedgerCode, tblchartofaccounts.Description,if(tblsales.ContactType='COMPANY','CORPORATE','RESIDENTIAL') as ClientType, tblcontract.CompanyGroup,"
                qry = qry + " tblcontract.ContractGroup as ContractGroup,"
                qry = qry + " tblsalesdetail.InvoiceNumber,tblsalesdetail.ValueBase, tblsalesdetail.GstBase"
                qry = qry + " FROM (((tblsales LEFT OUTER JOIN tblsalesdetail ON tblsales.InvoiceNumber=tblsalesdetail.InvoiceNumber)  LEFT OUTER JOIN tblservicerecord ON tblsalesdetail.RefType=tblservicerecord.RecordNo) LEFT OUTER JOIN  tblcontract ON tblservicerecord.ContractNo=tblcontract.ContractNo) LEFT OUTER JOIN tblchartofaccounts ON tblsalesdetail.LedgerCode=tblchartofaccounts.COACode"
                qry = qry + "  where tblsales.PostStatus='P' "
            ElseIf rbtnSelectDetSumm.SelectedValue = "2" Then
                qry = "SELECT "
                If Convert.ToString(Session("LocationEnabled")) = "Y" Then
                    qry = qry + "tblsales.Location as Branch, "
                ...
                qry = qry + " sum(tblsalesdetail.ValueBase) as ValueBase, sum(tblsalesdetail.GstBase) as GstBase"
                qry = qry + " FROM (((tblsales LEFT OUTER JOIN tblsalesdetail ON tblsales.InvoiceNumber=tblsalesdetail.InvoiceNumber)  LEFT OUTER JOIN tblservicerecord ON tblsalesdetail.RefType=tblservicerecord.RecordNo) LEFT OUTER JOIN  tblcontract ON tblservicerecord.ContractNo=tblcontract.ContractNo) LEFT OUTER JOIN tblchartofaccounts ON tblsalesdetail.LedgerCode=tblchartofaccounts.COACode"
                qry = qry + "  where tblsales.PostStatus='P' "
            End If
            ...
            If rbtnSelectDetSumm.SelectedValue = "2" Then 'summary
                qry = qry + " GROUP BY tblsalesdetail.LedgerCode, tblsalesdetail.SubLedgerCode, tblsalesdetail.ContractGroup, tblcontract.ContractGroup, tblcontract.CompanyGroup,tblsales.ContactType"
                '  qry = qry + " GROUP BY tblsalesdetail.LedgerCode"
            End If
        ...
        'If rbtnSelectDetSumm.SelectedValue = "2" Then 'summary
        '    If chkDistribution.Checked Then
        '        'qry = qry + "format(if(isnull(tblsalesdetail.costcode)=1 OR tblsalesdetail.costcode='',(tblsalesdetail.ValueBase),(tblsalesdetail.ValueBase*tblcontractvaluedistribution.percentage/100)),2) as ValueBase,"
        '        'qry = qry + "format(if(isnull(tblsalesdetail.costcode)=1 OR tblsalesdetail.costcode='',(tblsalesdetail.GstBase),(tblsalesdetail.GstBase*tblcontractvaluedistribution.percentage/100)),2) as GstBase"
        '        qry = qry + "format(if(isnull(tblsalesdetail.costcode)=1 OR tblsalesdetail.costcode='',sum(tblsalesdetail.ValueBase),sum(tblsalesdetail.ValueBase*tblcontractvaluedistribution.percentage/100)),2) as ValueBase,"
        '        qry = qry + "format(if(isnull(tblsalesdetail.costcode)=1 OR tblsalesdetail.costcode='',sum(tblsalesdetail.GstBase),sum(tblsalesdetail.GstBase*tblcontractvaluedistribution.percentage/100)),2) as GstBase"

        '        ' qry = qry + " format(sum(tblsalesdetail.ValueBase*tblcontractvaluedistribution.Percentage/100),2) as ValueBase, format(sum(tblsalesdetail.GstBase*tblcontractvaluedistribution.Percentage/100),2) as GstBase"
        '        qry = qry + " FROM (((tblsales LEFT OUTER JOIN tblsalesdetail ON tblsales.InvoiceNumber=tblsalesdetail.InvoiceNumber)  LEFT OUTER JOIN tblservicerecord ON tblsalesdetail.RefType=tblservicerecord.RecordNo) LEFT OUTER JOIN  tblcontract ON tblservicerecord.ContractNo=tblcontract.ContractNo) LEFT OUTER JOIN tblchartofaccounts ON tblsalesdetail.LedgerCode=tblchartofaccounts.COACode"
        '        qry = qry + " left outer join tblcontractvaluedistribution on tblsalesdetail.costcode=tblcontractvaluedistribution.contractno "
        ...
        '    End If
        '    qry = qry + "  where 1=1 "
        'ElseIf rbtnSelectDetSumm.SelectedValue = "1" Then 'detail
        '    If chkDistribution.Checked Then
        '        qry = qry + " if(isnull(tblsalesdetail.costcode)=1 or tblsalesdetail.costcode='',tblcontract.contractgroup,tblcontractvaluedistribution.ContractGroup) as ContractGroup,"
        '        qry = qry + "if(isnull(tblsalesdetail.costcode)=1 or tblsalesdetail.costcode='',100.00,tblcontractvaluedistribution.percentage) as Percentage,"
        '        qry = qry + " tblsalesdetail.InvoiceNumber,"
        '        qry = qry + "format(if(isnull(tblsalesdetail.costcode)=1 OR tblsalesdetail.costcode='',tblsalesdetail.ValueBase,(tblsalesdetail.ValueBase*tblcontractvaluedistribution.percentage/100)),2) as ValueBase,"
        '        qry = qry + "format(if(isnull(tblsalesdetail.costcode)=1 OR tblsalesdetail.costcode='',tblsalesdetail.GstBase,(tblsalesdetail.GstBase*tblcontractvaluedistribution.percentage)/100),2) as GstBase"
        '        qry = qry + " FROM (((tblsales LEFT OUTER JOIN tblsalesdetail ON tblsales.InvoiceNumber=tblsalesdetail.InvoiceNumber)  LEFT OUTER JOIN tblservicerecord ON tblsalesdetail.RefType=tblservicerecord.RecordNo) LEFT OUTER JOIN  tblcontract ON tblservicerecord.ContractNo=tblcontract.ContractNo) LEFT OUTER JOIN tblchartofaccounts ON tblsalesdetail.LedgerCode=tblchartofaccounts.COACode"
        '        qry = qry + " left outer join tblcontractvaluedistribution on tblsalesdetail.costcode=tblcontractvaluedistribution.contractno "
        ...
        ''    qry = qry + " tblsalesdetail.SubCode, tblsalesdetail.RefType, replace(replace(tblsalesdetail.description, char(10), ' '), char(13), ' ') as Description, tblsalesdetail.ValueBase, tblsalesdetail.GstBase, "
        ...
        ''End If
        ''qry = qry + " tblsales.GlPeriod,tblsales.Comments,tblsales.ValueBase+tblsales.GstBase-tblsales.creditBase-tblsales.receiptBase as balance"
        ''qry = qry + " FROM tblsales LEFT OUTER JOIN tblsalesdetail ON tblsales.InvoiceNumber=tblsalesdetail.InvoiceNumber where 1=1 AND RECURRINGINVOICE='N' AND DOCTYPE='ARIN'"
    ...
    Private Function GetDataSet() As DataTable
        ...
        sda.Fill(dt)
        InsertIntoTblWebEventLog("GetDataSet", dt.Rows.Count.ToString, "str")
        ...
        For Each dr As DataRow In dt.Rows
            If dr.Item("ValueBase").ToString <> DBNull.Value.ToString Then
                TotalValue = TotalValue + dr.Item("ValueBase")
            End If
            If dr.Item("GstBase").ToString <> DBNull.Value.ToString Then
                TotalGst = TotalGst + dr.Item("GstBase")
            End If
        Next

        drTotal.Item(0) = "Total"
        drTotal.Item("ValueBase") = TotalValue
        drTotal.Item("GstBase") = TotalGst
    ...
    Protected Sub btnExportToExcel_Click(sender As Object, e As EventArgs) Handles btnExportToExcel.Click
        ...
        If GetData() = True Then
            ...
            Dim attachment As String = "attachment; filename=SalesByLedger.xls"
    ...
    Public Sub WriteExcelWithNPOI(ByVal dt As DataTable, ByVal extension As String, criteria As String)
        ...
        Using exportData = New MemoryStream()
            ...
            Dim attachment As String = "attachment; filename=SalesByLedger" + criteria
            If extension = "xlsx" Then

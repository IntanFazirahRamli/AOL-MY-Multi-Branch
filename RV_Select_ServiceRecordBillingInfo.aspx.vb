...
Partial Class RV_Select_ServiceRecordBillingInfo
    ...
    Private Function GetData() As Boolean
        ...
        Dim qry As String = "Select B.CompanyGroup,B.ContractGroup,A.ContractNo,A.Status as ServiceStatus,A.SchServiceDate,"
        qry = qry + "A.SchServiceTime,A.SchServiceTimeOut,A.ServiceBy,A.TeamID,A.RecordNo,A.CustName,A.LocationID,A.ServiceName,"
        qry = qry + "A.CustAddress1,A.CustAddBuilding,A.CustAddPostal,A.Address1,A.AddBuilding,A.AddPostal,A.Contact,"
        qry = qry + "A.ContactPersonMobile,A.CustMobile,A.Contact1Tel,A.Email,A.ContactPerson2,A.Contact2Mobile,A.Contact1Tel2,"
        qry = qry + "A.Contact2Email,A.OtherEmail,A.BillingName,A.BillAddress,A.BillBuilding,A.BillStreet,A.BillCountry,"
        qry = qry + "A.BillPostal,B.Salesman,A.Scheduler,A.BillAmount as ServiceValue,B.AgreeValue as ContractValue,A.Notes,"
        qry = qry + "A.Comments,B.BillingFrequency,C.BilledAmount,C.LatestInvoiceNumber"
        qry = qry + " from tblServiceRecord A inner join tblContract B on A.ContractNo=B.ContractNo"
        qry = qry + " left outer join vwservicerecordbilledamount C on A.RecordNo=C.RecordNo"
        qry = qry + " where A.Status in ('O','P') "
        ...
        qry = qry + "order by A.ContractNo,A.ServiceDate,A.RecordNo;"
    ...
    Public Sub WriteExcelWithNPOI(ByVal dt As DataTable, ByVal extension As String)
        ...
        Using exportData = New MemoryStream()
            ...
            If extension = "xlsx" Then
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                Response.AddHeader("Content-Disposition", String.Format("attachment;filename={0}", "Service Listing with Contract & Billing Information" + txtCriteria.Text + ".xlsx"))
                Response.BinaryWrite(exportData.ToArray())
            ElseIf extension = "xls" Then
                Response.ContentType = "application/vnd.ms-excel"
                Response.AddHeader("Content-Disposition", String.Format("attachment;filename={0}", "Service Listing with Contract & Billing Information" + txtCriteria.Text + ".xls"))
